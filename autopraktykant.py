...
SCAN_INTERVAL = 0.6   # jak często skanować okna (sekundy)
COOLDOWN      = 1.5   # przerwa po obsłużeniu okna (sekundy)
...
def load_rules(path: str) -> list[dict]:
    ...
    for i, rule in enumerate(rules):
        ...
        if "class" not in rule:
            raise ValueError(f"{nr}: brak wymaganego pola 'class'")
        if "button" not in rule:
            raise ValueError(f"{nr}: brak wymaganego pola 'button'")
        if "title" not in rule and "title_regex" not in rule:
            raise ValueError(f"{nr}: wymagane pole 'title' lub 'title_regex'")
        if "title" in rule and "title_regex" in rule:
            raise ValueError(f"{nr}: podaj 'title' LUB 'title_regex', nie oba")
        for field in ("title_regex", "text_regex"):
            if field in rule:
                try:
                    re.compile(rule[field])
                except re.error as e:
                    raise ValueError(f"{nr}: błędny '{field}': {e}")
    log.info(f"Załadowano {len(rules)} reguł z '{path}'")
    return rules

# ── pomocnicze ────────────────────────────────────────────────────────────────

def safe_call(fn, default=None):
    # wywołuje fn(), zwraca default jeśli wyrzuci wyjątek
    try:
        return fn()
    except Exception:
        return default
...
def get_text_lines(win) -> list[str]:
    # zbiera wszystkie widoczne teksty z kontrolek okna
    lines = []
...
def matches_rule(win, rule: dict) -> bool:
    # sprawdza czy okno pasuje do reguły — klasa, tytuł, opcjonalnie tekst
...
def click_button(win, button_name: str) -> bool:
    ...
    for ctrl in win.descendants():
        ...
        # próba invoke — klika bez ruszania myszą
        try:
            ctrl.invoke()
        ...
        # fallback — click_input z przywróceniem pozycji kursora
        ...
        try:
            ...
            ctypes.windll.user32.SetCursorPos(pt.x, pt.y)  # przywróć kursor niezależnie od wyniku

    log.warning(f"    Nie znaleziono przycisku: '{button_name}'")
    return False

# ── wątek nasłuchujący komend ─────────────────────────────────────────────────

def watch_input(rules: list) -> None:
    # drugi wątek — czeka na komendy podczas gdy główny skanuje okna
    while True:
        ...
        if cmd == "r":
            try:
                ...
                log.info("Reguły przeładowane.")
            except (FileNotFoundError, ValueError, json.JSONDecodeError) as e:
                log.error(f"Błąd przeładowania reguł: {e}")
...
def monitor(rules: list[dict], total: list, by_rule: dict) -> None:
    ...
    while True:
        now     = time.time()
        handled = {h: t for h, t in handled.items() if now - t < 60}  # usuń stare uchwyty
        ...
        for win in desktop.windows():
            ...
            for idx, rule in enumerate(rules):
                if matches_rule(win, rule):
                    ...
                    log.info(f"[OKNO] class={cls} | title='{title}' | reguła #{idx + 1}")

                    if click_button(win, rule["button"]):
                        handled[handle] = now
                        total[0] += 1
                        by_rule[idx] = by_rule.get(idx, 0) + 1
                    break  # nie sprawdzaj kolejnych reguł jeśli już znalazłeś dopasowanie
...
if __name__ == "__main__":
    try:
        ...
        log.error(f"Nie znaleziono pliku reguł: '{RULES_FILE}'")
        input("ENTER żeby zamknąć...")
        raise SystemExit(1)
    except (ValueError, json.JSONDecodeError) as e:
        log.error(f"Błąd w pliku reguł: {e}")
        input("ENTER żeby zamknąć...")
        raise SystemExit(1)

    log.info("=== AUTOPRAKTYKANT URUCHOMIONY ===")
    log.info(f"Plik logów: {Path(LOG_FILE).resolve()}")
    log.info("'r' + ENTER żeby przeładować reguły | CTRL+C żeby zakończyć")

    total   = [0]  # lista zamiast liczby — żeby dało się modyfikować wewnątrz funkcji
    by_rule: dict[int, int] = {}
    start   = time.time()

    # uruchom wątek nasłuchujący komend (daemon=True — zamknie się razem z programem)
    ...
    while True:
        try:
            ...
            log.info("=" * 50)
            log.info(f"  Czas pracy:      {h}h {m}min")
            log.info(f"  Obsłużono okien: {total[0]}")
            for idx, count in sorted(by_rule.items()):
                name = rules[idx].get("title") or rules[idx].get("title_regex", f"reguła #{idx+1}")
                log.info(f"  Reguła #{idx + 1} ({name}): {count}x")
            log.info("=" * 50)
            break
        except Exception:
            log.error("Nieoczekiwany błąd — restart za 5s:")
